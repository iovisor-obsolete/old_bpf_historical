...
void ovs_dp_process_received_packet(struct vport *p, struct sk_buff *skb)
{
	...
	if (list_empty(rcu_dereference(dp->table)->mask_list))
		goto do_upcall;
...
do_upcall:
		...
		if (p->port_no < PLUM_MAX_PORTS &&
		    atomic_read(&plum->ports[p->port_no])) {
...
static void __dp_destroy(struct datapath *dp)
{
	...
	for (i = 0; i < DP_MAX_PLUMS; i++)
		bpf_dp_unregister_plum(dp->plums[i], PLUM_DATA | PLUM_TABLES);
